...
**To Make Logo -** `/logo  (Name)`
**To Make Square Logo - ** `/logosq (Name)`
...
async def logo(bot, message):
  try:
    ...
    await message.reply_photo(logo,caption="**🖼 Logo Generated By @BUDDY_FF**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
...
async def logo(bot, message):
  try:
    text = message.text.replace("logosq","").replace("/","").replace("@LOGO_MEKAR_BOT","").strip().upper()
    ...
    await message.reply_photo(logo,caption="**🖼 Logo Generated By @LOGO_MEKAR_BOT**",reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Upload As File 📁", callback_data=f"flogo {logo_id}")]]))
    await x.delete()
  except FloodWait:
    pass
  except Exception as e:
...
async def logo_doc(_,query):
  ...
  try:
    ...
    await query.message.reply_document(link,caption="**🖼 Logo Generated By @LOGO_MEKAR_BOT**")
  except FloodWait:
    pass
  except Exception as e:
    try:
